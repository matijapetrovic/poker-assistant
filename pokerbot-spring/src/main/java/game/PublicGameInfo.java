...
public final class PublicGameInfo implements GameInfo {
    ...
    private List<Integer> getPlayersContestingPot(Pot pot) {
        ...
        // There is only one eligible player and he folded, return his bet
        if (playersContestingPot.size() == 0 && pot.getEligiblePlayers().size() == 1) {
            playersContestingPot = new ArrayList<Integer>(pot.getEligiblePlayers());
        }
    ...
    public void update(Action act, int s) { //UNFINISHED
        ...
        observersFireBeforeActionEvent(s, act);
        ...
        if (act.isAllInPass()) {
            ...
            if (stage == Holdem.PREFLOP && lastToAct == smallBlindSeat && numRaises == 1) {
    ...
    private void observersFireBeforeActionEvent(int seat, Action action) {
        for (GameObserver gameObserver : gameObservers) {
            if (gameObserver instanceof ExtendedGameObserver) {
                ((ExtendedGameObserver) gameObserver).beforeActionEvent(seat, action);
            }
        }
    }
    ...
    public void observersFireGameOverEvent() {
        for (GameObserver gameObserver : gameObservers) {
            if (gameObserver instanceof ExtendedGameObserver) {
                ((ExtendedGameObserver) gameObserver).gameOverEvent(this);
            }
    ...
    public int seatPlayer(PublicPlayerInfo newPlayer) {
        int newSeat = -1;
        for (int seat = 0; seat < this.getNumSeats(); seat++) {
            PublicPlayerInfo oldPlayer = getPlayer(seat);
            if (oldPlayer == null || oldPlayer.isSittingOut()) {
                if (oldPlayer != null) {
        ...
        if (newSeat == -1) {
            ...
            newSeat = players.length - 1;
        }

        setPlayer(newSeat, newPlayer);
        newPlayer.setSittingOut(false);

        return newSeat;
    }

    public void removePlayer(PublicPlayerInfo player) {
        if (!player.isSittingOut()) {
            throw new IllegalArgumentException("Can only remove players that are sitting out!");
        }
        int seat = getPlayerSeat(player.getName());
        if (seat == lastToAct) {
            lastToAct = previousActivePlayer(lastToAct);
        }
        gameObservers.remove(player.getBot());
        players[seat] = null;
        player.setGame(null);
    }
}
